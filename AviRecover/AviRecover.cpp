...
bool setImageSize(AVI_file_header *pHdr, int width, int height);
void find_image_size(unsigned char *jpeg_image, unsigned long len, int *width, int *height);
...
void find_image_size(unsigned char *jpeg_image, unsigned long len, int *width, int *height) {
    ...
    jpeg_mem_src(&info, jpeg_image, len);
...
int _tmain(int argc, _TCHAR* argv[])
{
    ...
    while ((pFrame = read_frame(avi_stream)) != NULL)
    {
        ...
        if (streamData.framesCount == 1) {
            int width, height;
            find_image_size(pFrame->data, pFrame->size, &width, &height);
            printf("Setting image size to %d x %d\n", width, height);
            setImageSize(&hdr, width, height);
        }

        if (streamData.framesCount > 123000) {
            FILE *jpg;
